...
const botonAgregarMedicion = ref(false)
...
onMounted(async () => {
  ...
  tienePermiso('measures.add').then((res) => (botonAgregarMedicion.value = res))
})
...
<template>
  <div v-if="punto">
    ...
    <div>
      <router-link v-if="botonAgregarMedicion" :to="`/punto/${punto.id}/agregar-medicion`"
        >Agregar medición</router-link
      >
      <span v-if="botonAgregarMedicion"> | </span>
